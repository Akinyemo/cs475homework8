...
main( int argc, char *argv[ ] )
{
	...
	if( me == BOSS )
	{
		for( int dst = 0; dst < NumCpus; dst++ )
		{
			...
			MPI_Send( &BigSignal[dst*PPSize], ???, MPI_FLOAT, dst, 'A', MPI_COMM_WORLD );
		}
	}
	else
	{
		MPI_Recv( PPSignal, ???, MPI_FLOAT, ???, 'A', MPI_COMM_WORLD, &status );
	}
	...
	{
		MPI_Send( PPSums, ???, MPI_FLOAT, ???, 'C', MPI_COMM_WORLD );
	}
	...
	if( me == BOSS )
	{
		...
		for( int src = 0; src < NumCpus; src++ )
		{
			...
			MPI_Recv( tmpSums, ???, MPI_FLOAT, ???, 'C', MPI_COMM_WORLD, &status );
			for( int s = 0; s < MAXSHIFTS; s++ )
				BigSums[s] += tmpSums[s];
		}
	}
